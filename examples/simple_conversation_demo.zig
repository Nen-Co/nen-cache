...
pub fn main() !void {
    ...
    std.debug.print("🤖 NenCache Conversation Demo\n", .{});
    std.debug.print("============================\n", .{});
    std.debug.print("This demo shows conversations being stored in NenCache!\n\n", .{});

    // Simulate a conversation that gets stored
    const messages = [_][]const u8{ "Hello! How are you?", "What is NenCache?", "How fast is it?", "Can you remember what I said?", "Show me the stats" };
    ...
    for (messages) |msg| {
        ...
        const response = if (std.mem.indexOf(u8, msg, "hello") != null)
            ...
            const stats_msg = try std.fmt.allocPrint(allocator, "Stats: {d:.1}% hit rate, {d} operations, {d:.2} MB used", .{ stats.getHitRate() * 100.0, stats.total_gets + stats.total_sets, @as(f64, @floatFromInt(memory_stats.total_memory_bytes)) / (1024.0 * 1024.0) });
