...
const EnhancedKVCache = @import("../../src/cache/enhanced_kv_cache.zig").EnhancedKVCache;
const StaticCacheTier = @import("../../src/memory/static_cache.zig").StaticCacheTier;
...
pub const BenchmarkSuite = struct {
    ...
    fn benchmarkSetWithSize(self: *BenchmarkSuite, value_size: usize) !void {
        ...
        const result = BenchmarkResult.init("SET", .cpu, // Default tier for set operations
            self.config.iterations, total_time_ns, memory_stats.total_memory_bytes);
    ...
    fn benchmarkGetWithSize(self: *BenchmarkSuite, value_size: usize) !void {
        ...
        const result = BenchmarkResult.init("GET", .cpu, // Default tier for get operations
            self.config.iterations, total_time_ns, memory_stats.total_memory_bytes);
    ...
    pub fn benchmarkMixed(self: *BenchmarkSuite) !void {
        ...
        const result = BenchmarkResult.init("MIXED", .cpu, self.config.iterations, total_time_ns, memory_stats.total_memory_bytes);
    ...
    fn printSummary(self: *BenchmarkSuite) !void {
        ...
        for (self.results.items) |result| {
            stdout.print("\n{s}: {d:.0} ops/sec, {d:.2} ns avg, {d:.2} MB\n", .{ result.operation, result.throughput_ops_per_sec, result.average_time_ns, @as(f64, @floatFromInt(result.memory_usage_bytes)) / (1024.0 * 1024.0) }) catch {};
